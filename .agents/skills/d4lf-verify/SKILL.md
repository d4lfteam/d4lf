---
name: d4lf-verify
description: Run d4lf validation in a disciplined, targeted way. Use after code changes, especially Python, UI, importer, overlay, lint, profile model, or packaging edits. Avoid this skill for purely textual documentation-only edits unless validation is relevant.
---

Validation order:
1. Prefer the narrowest relevant validation for the touched code.
2. Run lint first when the task is mainly code-shape, callback-signature, validator, or refactor related.
3. Run targeted tests before broad suites when the affected area is known.
4. Use the repository environment locally with `uv run ...` unless you are inside CI or the environment is already prepared.
5. Do not claim verification you did not actually run.
