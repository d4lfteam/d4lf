...
from src.perception.tooltip.texture import _LONG_SEPARATOR_TEMPLATE_REFS
...
def test_production_locator_uses_untraced_bullet_search(mocker) -> None:
    ...
    plain_search = mocker.patch("src.perception.tooltip.texture.find_bullets_for_templates", return_value=[bullet])
...
def test_production_locator_reuses_supplied_short_separator(mocker) -> None:
    ...
    separator_search = mocker.patch(
        "src.perception.tooltip.texture.find_seperator_short", side_effect=AssertionError("separator should be reused")
    )
    mocker.patch("src.perception.tooltip.texture.find_seperator_long", return_value=long_separator)
    affix = Affix(name="life")
...
def test_locator_accepts_duplicate_matches_for_same_affix_row(mocker) -> None:
    ...
    mocker.patch("src.perception.tooltip.texture.find_bullets_for_templates", return_value=[bullet])
    affix = Affix(name="life")
