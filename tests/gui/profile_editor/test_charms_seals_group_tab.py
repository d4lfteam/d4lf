...
def test_charms_tab_close_tab_safely(qapp, mock_ini_loader):
    ...
    assert len(tab.charms_model) == 3
    assert tab.item_names == ["Charm1", "Charm2", "Charm3"]
    ...
    assert tab.item_names == ["Charm1", "Charm3"]
    assert tab.tab_widget.count() == 2
    assert len(tab.charms_model) == 2
    # Ensure remaining models have correct keys
    assert "Charm1" in tab.charms_model[0].root
    assert "Charm3" in tab.charms_model[1].root
...
def test_seals_tab_close_tab_safely(qapp, mock_ini_loader):
    ...
    assert len(tab.seals_model) == 3
    assert tab.item_names == ["Seal1", "Seal2", "Seal3"]

    # Close Seal2 (index 1)
    tab.close_tab(1)

    assert tab.item_names == ["Seal1", "Seal3"]
    assert tab.tab_widget.count() == 2
    assert len(tab.seals_model) == 2
    assert "Seal1" in tab.seals_model[0].root
    assert "Seal3" in tab.seals_model[1].root
