...
def test_load_validation_guidance_shows_critical(qapp, monkeypatch, tmp_path: Path):
    ...
    fake_session = _FakeSession(
        catalog=_catalog(tmp_path), load_result=ValidationError(message="bad profile", guidance="guidance text")
    )
...
def test_load_yaml_error_does_not_show_critical(qapp, monkeypatch, tmp_path: Path):
    monkeypatch.setattr(profile_tab_module, "Dataloader", _FakeDataloader)
    monkeypatch.setattr(profile_tab_module, "ProfileEditor", _FakeEditor)
    fake_session = _FakeSession(catalog=_catalog(tmp_path), load_result=YamlError(message="broken yaml"))
    monkeypatch.setattr(profile_tab_module, "ProfileSession", lambda **_kwargs: fake_session)
...
def test_load_empty_error_does_not_show_critical(qapp, monkeypatch, tmp_path: Path):
    monkeypatch.setattr(profile_tab_module, "Dataloader", _FakeDataloader)
    monkeypatch.setattr(profile_tab_module, "ProfileEditor", _FakeEditor)
    fake_session = _FakeSession(catalog=_catalog(tmp_path), load_result=EmptyError(message="empty profile"))
    monkeypatch.setattr(profile_tab_module, "ProfileSession", lambda **_kwargs: fake_session)
