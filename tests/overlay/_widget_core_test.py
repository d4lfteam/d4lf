...
from src.overlay._widget_core import _OverlayCore
...
def test_save_settings_persists_relative_position_and_flags(monkeypatch):
    ...
    monkeypatch.setattr("src.overlay._widget_core.game_window_roi", lambda: {"left": 10, "top": 20})
    saved = []
    monkeypatch.setattr("src.overlay._widget_core.save_info_settings", lambda values: saved.append(values))
