...
def test_toggle_off_on_twice_never_leaves_two_live_instances(fake_windows_overlay):
    for _ in range(2):
        _lifecycle.open_overlay()
        assert _lifecycle.is_open()
...
@pytest.mark.skipif(sys.platform != "win32", reason="Windows-only overlay test")
def test_concurrent_open_requests_only_create_one_instance(fake_windows_overlay):
...
@pytest.mark.skipif(sys.platform != "win32", reason="Windows-only overlay test")
def test_request_close_without_an_open_overlay_is_a_no_op(fake_windows_overlay):
    _lifecycle.request_close()
