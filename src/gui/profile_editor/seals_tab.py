...
class SealGroupEditor(QWidget):
    ...
    def setup_ui(self):
        ...
        # Affix Pool (no inherent pool for seals)
...
class SealsTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.toolbar = QToolBar("SealsToolBar", self)
        ...
        remove_item_button.setText("Remove Seal")
    ...
    def add_item_type(self):
        dialog = CreateCharmOrSeal(self.item_names, is_charm=False, parent=self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            item = dialog.get_value()
            for item_name in item.root:
                group = SealGroupEditor(item)
                self.item_names.append(item_name)
                self.tab_widget.addTab(group, item_name)
                self.seals_model.append(item)
            return

    def close_tab(self, index):
        self.item_names.pop(index)
        self.tab_widget.removeTab(index)
        self.seals_model.pop(index)
    ...
    def set_all_min_greater_affix(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            for i in range(self.tab_widget.count()):
                tab: SealGroupEditor = self.tab_widget.widget(i)
    ...
    def convert_all_to_min_percent_of_affix(self):
        ...
        if isinstance(current_tab, SealGroupEditor):
