...
class WNDCLASSW(ctypes.Structure):
    _fields_ = [("style", wt.UINT), ("lpfnWndProc", WNDPROCTYPE), ("cbClsExtra", ctypes.c_int),
                ...
                ("hCursor", HCURSOR), ("hbrBackground", HBRUSH), ("lpszMenuName", wt.LPCWSTR),
...
# --- API Signatures ---
kernel32.GetModuleHandleW.argtypes = [wt.LPCWSTR]; kernel32.GetModuleHandleW.restype = HINSTANCE
user32.RegisterClassW.argtypes = [ctypes.POINTER(WNDCLASSW)]; user32.RegisterClassW.restype = wt.ATOM
user32.CreateWindowExW.argtypes = [wt.DWORD, wt.LPCWSTR, wt.LPCWSTR, wt.DWORD, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int, HWND, HMENU, HINSTANCE, LPVOID]; user32.CreateWindowExW.restype = HWND
user32.DefWindowProcW.argtypes = [HWND, wt.UINT, WPARAM, LPARAM]; user32.DefWindowProcW.restype = LRESULT
user32.UpdateLayeredWindow.argtypes = [HWND, HDC, ctypes.POINTER(wt.POINT), ctypes.POINTER(wt.SIZE), HDC, ctypes.POINTER(wt.POINT), wt.COLORREF, ctypes.POINTER(BLENDFUNCTION), wt.DWORD]; user32.UpdateLayeredWindow.restype = wt.BOOL
user32.GetDC.argtypes = [HWND]; user32.GetDC.restype = HDC
user32.ReleaseDC.argtypes = [HWND, HDC]; user32.ReleaseDC.restype = ctypes.c_int
...
user32.GetKeyState.argtypes = [ctypes.c_int]; user32.GetKeyState.restype = wt.SHORT
user32.GetSystemMetrics.argtypes = [ctypes.c_int]; user32.GetSystemMetrics.restype = ctypes.c_int
user32.LoadCursorW.argtypes = [HINSTANCE, wt.LPCWSTR]; user32.LoadCursorW.restype = HCURSOR
user32.GetMessageW.argtypes = [ctypes.POINTER(wt.MSG), HWND, wt.UINT, wt.UINT]; user32.GetMessageW.restype = wt.BOOL
user32.TranslateMessage.argtypes = [ctypes.POINTER(wt.MSG)]; user32.TranslateMessage.restype = wt.BOOL
user32.DispatchMessageW.argtypes = [ctypes.POINTER(wt.MSG)]; user32.DispatchMessageW.restype = LRESULT
user32.SetCapture.argtypes = [HWND]; user32.SetCapture.restype = HWND
user32.ReleaseCapture.argtypes = []; user32.ReleaseCapture.restype = wt.BOOL
user32.GetCursorPos.argtypes = [ctypes.POINTER(wt.POINT)]; user32.GetCursorPos.restype = wt.BOOL
user32.SetCursor.argtypes = [HCURSOR]; user32.SetCursor.restype = HCURSOR
user32.SetWindowPos.argtypes = [HWND, HWND, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int, wt.UINT]; user32.SetWindowPos.restype = wt.BOOL
...
gdi32.DeleteDC.argtypes = [HDC]; gdi32.DeleteDC.restype = wt.BOOL
gdi32.DeleteObject.argtypes = [HGDIOBJ]; gdi32.DeleteObject.restype = wt.BOOL
gdi32.CreateDIBSection.argtypes = [HDC, ctypes.POINTER(BITMAPINFO), wt.UINT, ctypes.POINTER(ctypes.c_void_p), HANDLE, wt.DWORD]; gdi32.CreateDIBSection.restype = HBITMAP
...
def nodes_to_grid(nodes_441):
    ...
    for y in range(GRID):
        row = []
        for x in range(GRID):
            row.append(bool(nodes_441[y * GRID + x]))
        grid.append(row)
    return grid

def rotate_grid(grid, deg: int):
    if deg == 90: return [list(reversed(col)) for col in zip(*grid)]
    if deg == 180: return [list(reversed(r)) for r in reversed(grid)]
    if deg == 270: return [list(col) for col in reversed(list(zip(*grid)))]
    return grid
...
def _load_builds_from_file(preset_file: str, name_tag: str | None = None):
    ...
    if not builds:
        raise ValueError(f"No valid builds in {preset_file}")
...
def load_builds_from_path(preset_path: str):
    ...
    if p.is_dir():
        ...
        if not files:
            raise ValueError("Folder contains no .json files")
        multi = len(files) > 1
        builds = []
        for fp in files:
            try:
                ...
                # Ignore JSONs that don't match expected structure
                continue
        if not builds:
            raise ValueError("No valid builds found in folder")
        return builds
    ...
    for y in range(GRID):
        for x in range(GRID):
            if grid[y][x]:
                ...
                d.rectangle((px + inset, py + inset, px + cell_size - inset, py + cell_size - inset),
                            fill=C_NODE_ACTIVE, outline=None)
                d.rectangle((px + inset, py + inset, px + cell_size - inset, py + cell_size - inset),
                            outline=(200, 255, 200, 100), width=1)

    # 4. THICK Outer Frame
    frame_thick = 5
    d.rectangle((gx0 - 1, gy0 - 1, gx0 + grid_px + 1, gy0 + grid_px + 1), 
                outline=C_GRID_FRAME_BG, width=frame_thick + 2)
    d.rectangle((gx0, gy0, gx0 + grid_px, gy0 + grid_px), 
                outline=C_GRID_FRAME, width=frame_thick)
...
def render_list_window(state):
    ...
    for i, item in enumerate(data):
        ...
        if not selecting and item.get("Glyph"): label += f" ({item.get('Glyph')})"
        
        y = y_start + i * (ITEM_H + 4)
        bg, border, txt = C_ITEM_BG, C_ITEM_BORDER, C_TEXT_DIM
        
        if i == active_idx: bg, border, txt = (40, 35, 20, 220), C_GOLD, C_TEXT
        elif i == state.hover: border, txt = (150, 150, 150, 255), C_TEXT
...
def update_window(hwnd, img, x, y):
    ...
    user32.UpdateLayeredWindow(hwnd, hdc_screen, ctypes.byref(pt_dst), ctypes.byref(sz),
    ...
    gdi32.SelectObject(hdc_mem, old); gdi32.DeleteObject(hbmp); gdi32.DeleteDC(hdc_mem); user32.ReleaseDC(None, hdc_screen)
...
class AppState:
    ...
    def save_config(self):
        cfg = {"list_pos": self.list_pos, "grid_pos": self.grid_pos, "cell_size": self.cell_size, "minimized": self.minimized}
        try:
...
def WndProcList(hwnd, msg, w, l):
    ...
    if msg == WM_MOUSEMOVE:
        ...
        if y > HEADER_H:
            ...
            if nh != state.hover: state.hover = nh; redraw_all()
        elif state.hover != -1: state.hover = -1; redraw_all()
        return 0
    ...
    if msg == WM_LBUTTONDOWN:
        ...
        if x < 28 and y < 28: 
            state.minimized = not state.minimized; state.save_config(); redraw_all(); return 0
...
@WNDPROCTYPE
def WndProcGrid(hwnd, msg, w, l):
    if msg == WM_NCHITTEST: return HTCLIENT
    
    if msg == WM_MOUSEWHEEL:
        delta = ctypes.c_short(w >> 16).value
    ...
    if msg == WM_LBUTTONDOWN:
        user32.SetFocus(hwnd); user32.SetCapture(hwnd)
        state.dragging = True
        pt = wt.POINT(); user32.GetCursorPos(ctypes.byref(pt))
        state.drag_start = (pt.x, pt.y); state.drag_offset = state.grid_pos
    ...
    if msg == WM_MOUSEMOVE:
        if state.dragging:
            pt = wt.POINT(); user32.GetCursorPos(ctypes.byref(pt))
            dx = pt.x - state.drag_start[0]; dy = pt.y - state.drag_start[1]
    ...
    if msg == WM_LBUTTONUP:
        if state.dragging:
            state.dragging = False; user32.ReleaseCapture(); state.save_config()
    ...
    if msg == WM_KEYDOWN:
        ...
        if w == VK_LEFT: state.grid_pos = (gx - step, gy)
        elif w == VK_RIGHT: state.grid_pos = (gx + step, gy)
        elif w == VK_UP: state.grid_pos = (gx, gy - step)
        elif w == VK_DOWN: state.grid_pos = (gx, gy + step)
        if w in (VK_LEFT, VK_RIGHT, VK_UP, VK_DOWN): state.save_config(); redraw_all()
...
def run_paragon_overlay(preset_path: str | None = None) -> None:
    ...
    try:
        ...
        _msgbox(
            ...
            f"Konnte Paragon JSON nicht laden.\n\nQuelle: {preset}\n\nFehler: {e}",
        )
        return
    ...
    wc = WNDCLASSW(style=0, lpfnWndProc=WndProcList, hInstance=state.h_inst,
                   hCursor=user32.LoadCursorW(state.h_inst, wt.LPCWSTR(IDC_ARROW)), lpszClassName="D4ListCls")
    ...
    state.hwnd_list = user32.CreateWindowExW(ex_style, "D4ListCls", "List", WS_POPUP|WS_VISIBLE, 
                                             0, state.list_pos[1], 400, 600, None, None, state.h_inst, None)
    state.hwnd_grid = user32.CreateWindowExW(ex_style, "D4GridCls", "Grid", WS_POPUP|WS_VISIBLE, 
                                             state.grid_pos[0], state.grid_pos[1], 800, 800, None, None, state.h_inst, None)
