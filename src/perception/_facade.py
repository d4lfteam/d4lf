...
if TYPE_CHECKING:
    ...
    from ._matching_models import SearchResult, TemplateReferences
...
def capture(force_new: bool = False):
    from ._capture import Cam  # ruff:ignore[import-outside-top-level]
...
def create_template_query(**kwargs) -> TemplateQuery:
    from ._matching_models import SearchArgs  # ruff:ignore[import-outside-top-level]
...
def crop_image(image, roi):
    from ._image import crop  # ruff:ignore[import-outside-top-level]

    return crop(image, roi)


def alpha_mask(image):
    from ._image import alpha_to_mask  # ruff:ignore[import-outside-top-level]

    return alpha_to_mask(image)


def compare_image_histograms(image_a, image_b):
    from ._image import compare_histograms  # ruff:ignore[import-outside-top-level]

    return compare_histograms(image_a, image_b)


def center_of_roi(roi):
    from ._roi import get_center  # ruff:ignore[import-outside-top-level]

    return get_center(roi)


def grid_rois(roi, rows: int, columns: int):
    from ._roi import to_grid  # ruff:ignore[import-outside-top-level]
