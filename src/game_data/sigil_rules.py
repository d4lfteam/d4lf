...
class _ItemLike(Protocol):
    ...
    @property
    def affixes(self) -> Sequence[_AffixLike]: ...

    @property
    def inherent(self) -> Sequence[_AffixLike]: ...
...
class SigilRules:
    def __init__(self) -> None:
        sigil_data = GameCatalog().affix_sigil_dict_all
    ...
    def for_item(self, item: _ItemLike) -> SigilItem:
        affix_names = tuple(affix.name for affix in tuple(item.affixes) + tuple(item.inherent))
